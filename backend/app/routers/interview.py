...
def _context_for(user: models.User) -> str:
    ...
    return rag.search_job(user.target_role or "")
...
def start_interview(user: models.User = Depends(current_user), db: Session = Depends(get_db)):
    try:
        ...
        questions = json_completion(
            "You are a technical interviewer. Return only JSON.",
            f"Target role: {user.target_role or 'not provided'}\nCandidate skills: {[s.skill for s in user.skills]}\nRelevant job context: {context[:5000]}\nGenerate exactly five concise, distinct interview questions: two role-specific technical questions, one project question, one behavioral question, and one problem-solving question. Return {{\"questions\": [string, string, string, string, string]}}.",
        )["questions"]
        ...
        raise HTTPException(status_code=502, detail="The interview model returned an unusable result") from exc
