...
def get_roadmap(user: models.User = Depends(current_user)):
    ...
    try:
        result = json_completion(
            ...
            f"Target role: {user.target_role}\nCurrent skills: {[s.skill for s in user.skills]}\nCareer context: {rag.search_job(user.target_role)[:5000]}\n"
            "Create a focused 4-week roadmap. Return {\"weeks\": [{\"label\": \"Week 1\", \"topic\": string, \"progress\": 0}]}. Every progress value must be 0.",
        )
        weeks = result["weeks"]
